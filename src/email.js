...
const REMETENTE = process.env.EMAIL_FROM || 'WhatsApp Grupos <noreply@whatsappgrupos.site>';
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:10px;overflow:hidden;max-width:600px;width:100%;box-shadow:0 8px 30px rgba(18,63,40,0.08);">
        <tr><td style="background:#123f28;padding:24px 32px;">
          <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700;">WhatsApp Grupos</h1>
          <p style="margin:6px 0 0;color:#d9f8e5;font-size:14px;">Diretório brasileiro de comunidades no WhatsApp</p>
        </td></tr>
        ...
        <tr><td style="background:#f9f9f9;padding:16px 32px;border-top:1px solid #e5e5e5;">
          <p style="margin:0;color:#777;font-size:12px;line-height:1.5;">
            Este e-mail foi enviado automaticamente pelo WhatsApp Grupos. Não responda esta mensagem.<br>
            <a href="${SITE_URL}" style="color:#1a472a;font-weight:600;">Acessar WhatsApp Grupos</a>
          </p>
        </td></tr>
      </table>
    </td></tr>
...
async function emailGrupoAprovado(para, nomeGrupo, linkWhatsApp) {
  ...
  const html = htmlBase('Seu grupo foi aprovado - WhatsApp Grupos', `
    <h2 style="margin:0 0 16px;color:#1a472a;font-size:20px;">🎉 Seu grupo foi aprovado!</h2>
    <p style="color:#444;line-height:1.6;margin:0 0 10px;">O grupo <strong>${nomeSeguro}</strong> foi aprovado pela nossa equipe e já pode aparecer no diretório do WhatsApp Grupos.</p>
  ...
  return enviarEmail(para, 'Seu grupo foi aprovado - WhatsApp Grupos', html);
}
...
async function emailGrupoRejeitado(para, nomeGrupo, motivo) {
  ...
  const html = htmlBase('Seu grupo não foi aprovado - WhatsApp Grupos', `
  ...
  return enviarEmail(para, 'Seu grupo não foi aprovado - WhatsApp Grupos', html);
}

async function emailBoasVindas(para, nome) {
  const nomeSeguro = escaparHtml(nome || '');

  const html = htmlBase('Bem-vindo ao WhatsApp Grupos', `
    <h2 style="margin:0 0 16px;color:#1a472a;font-size:20px;">Bem-vindo, ${nomeSeguro}!</h2>
    <p style="color:#444;line-height:1.6;margin:0 0 10px;">Sua conta foi criada com sucesso no WhatsApp Grupos.</p>
    <p style="color:#444;line-height:1.6;margin:0;">Agora você pode enviar comunidades de WhatsApp para divulgação gratuita. Após o envio, nossa equipe fará a análise e você receberá uma resposta por e-mail.</p>
    <div style="margin:24px 0;">
      <a href="${SITE_URL}/pages/enviar-grupo.html" style="background:#1a472a;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:600;display:inline-block;">
  ...
  return enviarEmail(para, 'Bem-vindo ao WhatsApp Grupos', html);
}
